...
@interface ACXWhiteBalanceComponent ()
...
@implementation ACXWhiteBalanceComponent
...
{
	cv::Mat *image = [bufferManager mostRecentDataInBgrBuffer];
	...
	for (int channel=0; channel<MIN(3,image->channels()); ++channel)
	{
		[ACXWhiteBalanceComponent getRemapForHistogram:self.histograms[channel] withTotal:image->total() remapA:a aIndex:channel remapB:b bIndex:channel];
	}
	...
	[bufferManager setBgrBuffer:image];
	delete [] a;
	delete [] b;
}
